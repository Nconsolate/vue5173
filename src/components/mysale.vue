    ...
    <div class="games-box"> 
        <router-link
         ...
         v-for="game in games" :key="game.id"
          :to=" '/mysale/' +'goodsList?CategoryId='+game.catagoryId">
         ...
         <div class="boxs">
             <ul class="game-py">
             <li 
              @click="firstPy(['m2',null]);isHot=['1']" 
              :class="{active:gamePy===null}"
              ><i class="fa fa-thumbs-up"
              ></i></li>
             <li
              v-for="py in pys" :key="py"  @click="firstPy(['m2',[py]]);isHot=null;pageNum=1"
...
SelectNavHead
 ...
 return{
    ...
    pageNum:1
     ...
     computed:{
      gameType() {
           return this.$store.state.m2sk===true?"2":"1";
       },
        gamePy() {
           return this.$store.state.firstPYM2;
       },
       },
    ...
    this.$http.post(`/api${this.$api.GETGAMEPY}`,
          ...
          postSelectNav(){        
           this.$http.post(`/api${this.$api.GOODSEARCH}`,
              ...
              "accurateMap":{"game_type":[this.gameType],"first_py":this.$store.state.firstPYM2,"is_hot":this.isHot}}
          ).then( res => {
